...
st.markdown("<h1 style='text-align: center;'>WDRR Sequence Calculator for ES</h1>", unsafe_allow_html=True)
...
@st.cache_data
def load_data():
    df = pd.read_csv("ES_WDRR_From_2008_FullCSV.csv")
...
# WEEKLY PARTIAL MODEL DISTRIBUTION SECTION
st.divider()
st.markdown("<h2 style='text-align: center;'>🧩 Weekly Partial Model Distribution</h2>", unsafe_allow_html=True)

# Prepare unique values for both scenarios
pt1_vals = sorted(filtered1["Day 2 Type 2"].dropna().unique())
pm1_vals = sorted(filtered1["Day 2 Model"].dropna().unique())
pt2_vals = sorted(filtered2["Day 2 Type 2"].dropna().unique())
pm2_vals = sorted(filtered2["Day 2 Model"].dropna().unique())

# Layout structure
col_p1a, col_p1b, col_p2a, col_p2b = st.columns(4)
...
with col_p1b:
    st.subheader("Day 2 Model (Scenario 1)")
    s1_model_selected = st.multiselect(
        "Select values to include:",
        pm1_vals,
        default=pm1_vals,
        key="s1_model_multi",
...
with col_p2b:
    st.subheader("Day 2 Model (Scenario 2)")
    s2_model_selected = st.multiselect(
        "Select values to include:",
        pm2_vals,
        default=pm2_vals,
        key="s2_model_multi",
...
# Filter based on both fields together for cross-filtering effect
part_filtered1 = filtered1[
    filtered1["Day 2 Type 2"].isin(s1_type2_selected) & filtered1["Day 2 Model"].isin(s1_model_selected)
]
part_filtered2 = filtered2[
    filtered2["Day 2 Type 2"].isin(s2_type2_selected) & filtered2["Day 2 Model"].isin(s2_model_selected)
]

# --- Plotting charts (Scenario 1) ---
with col_p1a:
    counts = part_filtered1["Day 2 Type 2"].value_counts().sort_index()
    percentages = (counts / counts.sum() * 100).round(1)
    fig = px.bar(
    ...
    fig.update_traces(marker_color='#09AEB2', textposition='outside')
    fig.update_layout(yaxis_range=[0, 100])
    st.plotly_chart(fig, use_container_width=True, key="s1_type2_plot")

with col_p1b:
    counts = part_filtered1["Day 2 Model"].value_counts().sort_index()
    percentages = (counts / counts.sum() * 100).round(1)
    fig = px.bar(
        x=percentages.index, y=percentages.values,
        labels={'x': 'Model', 'y': '%'},
    ...
    fig.update_traces(marker_color='#09AEB2', textposition='outside')
    fig.update_layout(yaxis_range=[0, 100])
    st.plotly_chart(fig, use_container_width=True, key="s1_model_plot")

# --- Plotting charts (Scenario 2) ---
with col_p2a:
    counts = part_filtered2["Day 2 Type 2"].value_counts().sort_index()
    percentages = (counts / counts.sum() * 100).round(1)
    fig = px.bar(
        x=percentages.index, y=percentages.values,
        labels={'x': 'Type 2', 'y': '%'},
    ...
    fig.update_traces(marker_color='mediumslateblue', textposition='outside')
    fig.update_layout(yaxis_range=[0, 100])
    st.plotly_chart(fig, use_container_width=True, key="s2_type2_plot")

with col_p2b:
    counts = part_filtered2["Day 2 Model"].value_counts().sort_index()
    percentages = (counts / counts.sum() * 100).round(1)
    fig = px.bar(
        x=percentages.index, y=percentages.values,
        labels={'x': 'Model', 'y': '%'},
    ...
    fig.update_traces(marker_color='mediumslateblue', textposition='outside')
    fig.update_layout(yaxis_range=[0, 100])
    st.plotly_chart(fig, use_container_width=True, key="s2_model_plot")
...
# Scenario 1 Weekly Model Distribution (excluding Day 2 Type 2)
day_columns = ["Day 2 Model", "Day 3 Model", "Day 4 Model", "Day 5 Model"]
s1_cols = st.columns(len(day_columns))
for i, day in enumerate(day_columns):
    with s1_cols[i]:
        model_filtered1 = plot_model_distribution(
            model_filtered1, day, "", key_prefix=f"s1_{day.replace(' ', '_')}", color=scenario1_color
        )


s2_cols = st.columns(4)

# Scenario 2 Weekly Model Distribution (excluding Day 2 Type 2)
day_columns = ["Day 2 Model", "Day 3 Model", "Day 4 Model", "Day 5 Model"]
s2_cols = st.columns(len(day_columns))
for i, day in enumerate(day_columns):
    with s2_cols[i]:
        model_filtered2 = plot_model_distribution(
            model_filtered2, day, "", key_prefix=f"s2_{day.replace(' ', '_')}", color=scenario2_color
        )
