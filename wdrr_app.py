...
# Reusable chart function that returns filtered df
...
filtered1 = df.copy()
filtered2 = df.copy()
for val, col in [(conf1, "Confirmation"), (conf_tf1, "Conf True/False"), (model1, "Model"), (start_day1, "Start Day"), (start_hl1, "Start High/Low.1"), (start_sess1, "Start session")]:
    if val not in ["", "All"]:
        filtered1 = filtered1[filtered1[col] == val]
for val, col in [(conf2, "Confirmation"), (conf_tf2, "Conf True/False"), (model2, "Model"), (start_day2, "Start Day"), (start_hl2, "Start High/Low.1"), (start_sess2, "Start session")]:
    if val not in ["", "All"]:
...
# High/Low filters: get selections independently but apply together
hl1_default_high = sorted(filtered1["High Day"].dropna().unique())
hl1_default_low = sorted(filtered1["Low Day"].dropna().unique())
hl2_default_high = sorted(filtered2["High Day"].dropna().unique())
hl2_default_low = sorted(filtered2["Low Day"].dropna().unique())
...
# WEEKLY MODEL SECTION (independent copy for cross-filtering only in this section)
st.divider()
st.markdown("<h2 style='text-align: center;'>📊 Weekly Model Distribution</h2>", unsafe_allow_html=True)
model_filtered1 = filtered1.copy()
model_filtered2 = filtered2.copy()

model_pairs = [("Day 2 Model", "Day 3 Model"), ("Day 4 Model", "Day 5 Model")]
for pair in model_pairs:
    left_col1, left_col2, right_col1, right_col2 = st.columns(4)
    with left_col1:
        model_filtered1 = plot_model_distribution(model_filtered1, pair[0], "Scenario 1", key_prefix=f"s1_{pair[0].replace(' ', '_')}")
    with left_col2:
        model_filtered1 = plot_model_distribution(model_filtered1, pair[1], "Scenario 1", key_prefix=f"s1_{pair[1].replace(' ', '_')}")
    with right_col1:
        model_filtered2 = plot_model_distribution(model_filtered2, pair[0], "Scenario 2", key_prefix=f"s2_{pair[0].replace(' ', '_')}")
    with right_col2:
        model_filtered2 = plot_model_distribution(model_filtered2, pair[1], "Scenario 2", key_prefix=f"s2_{pair[1].replace(' ', '_')}")

# COMPARISON SECTION (based on filtered1 counts only)
st.divider()
