...
def generate_outcome_summary(filtered_df, scenario_title):
    ...
    plt.bar(outcome_summary["Outcome"], outcome_summary["Percentage"])
...
with c2:
    st.markdown(f"<p style='text-align: center;'><strong>Scenario 1:</strong> {pct1}% of total ({count1} datasets)</p>", unsafe_allow_html=True)
    st.markdown(f"<p style='text-align: center;'><strong>Scenario 2:</strong> {pct2}% of total ({count2} datasets)</p>", unsafe_allow_html=True)
