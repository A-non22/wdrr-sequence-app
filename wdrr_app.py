...
# Scenario 1 Weekly Model Distribution with Type 2 insert
day_columns = ["Day 2 Model", "Day 2 Type 2", "Day 3 Model", "Day 4 Model", "Day 5 Model"]
s1_cols = st.columns(5)
for i, day in enumerate(day_columns):
    with s1_cols[i]:
        if day == "Day 2 Type 2":
            # Use filtered data from Day 2 Model selection, but do not apply back
            temp_filtered1 = model_filtered1.copy()
            plot_model_distribution(temp_filtered1, day, "", key_prefix=f"s1_{day.replace(' ', '_')}", color=scenario1_color)
        else:
...
# Scenario 2 Weekly Model Distribution with Type 2 insert
day_columns = ["Day 2 Model", "Day 2 Type 2", "Day 3 Model", "Day 4 Model", "Day 5 Model"]
s2_cols = st.columns(5)
for i, day in enumerate(day_columns):
    with s2_cols[i]:
        if day == "Day 2 Type 2":
            temp_filtered2 = model_filtered2.copy()
            plot_model_distribution(temp_filtered2, day, "", key_prefix=f"s2_{day.replace(' ', '_')}", color=scenario2_color)
        else:
